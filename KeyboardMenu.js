class KeyboardMenu {
    ...
    setOptions(options) {
        ...
        this.element.innerHTML = this.options.map((option, index) => {
            ...
            return (`
                <div class="option">
                    <button ${disabledAttr} data-button="${index}" data-desciption="${option.description}">
                    ${option.label}
                    </button>
                    <span class="right">${option.right ? optionright() : ""}</span>
                </div>
             `)
        }).join("");
    ...
    init(container) {
        ...
        this.up = new KeyPressLisener("ArrowUp", () => {
        ...
        this.down = new KeyPressLisener("ArrowDown", () => {
            const current = Number(this.prevFocus.getAttribute("data-button"));
